...
def FB_translate(model_path, src_lang, tgt_lang):
    # print(sys.argv[1:])

    # generate parser / parse parameters
    parser = get_parser()
    # params = parser.parse_args()
    params = parser.parse_args(["--src_lang", src_lang, "--tgt_lang", tgt_lang, "--model_path", model_path])
    print(params)

    # ROOT = "/app/"
    # params.model_path = ROOT + params.model_path
    ...
    # TO DO: LOOKS LIKE THIS IS WHAT TAKES THE LONGEST TIME
    translator = Translator(params)
 
    # read input code from stdin
    src_sent = []
    inputFile = open("/app/server/outputProcessing/outputFiles/inputFile.java")
    ...
    completeName = ROOT_DIR + save_path + ".py"
    print("COMPLETE NAME:", completeName)

    outputFile = open(completeName, "w")
    outputFile.truncate(0) # clear the previous file contents
